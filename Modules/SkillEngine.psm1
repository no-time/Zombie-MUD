...
function Invoke-SkillRound {
    ...
    if ($SkillName -ieq 'Brains!') {
        ...
        if ($Player.LearnedSkills -contains "Feeding Frenzy" -and (Get-Random -Min 1 -Max 101) -le 15) {
            ...
            if ($null -ne $ExistingFrenzy) { $ExistingFrenzy.Duration = 5 } else { $Player.ActiveEffects += [PSCustomObject]@{ Name="Feeding Frenzy"; Duration=5; DoT=0; Modifiers=@{} } }
    ...
    elseif ($SkillName -ieq 'Focus') {
        $SPCost = 3
        if ($IsFreeCast) { $SPCost = 0 }
        if ($Player.SP -lt $SPCost -and $Player.Class -ne "Vampire") { return "Needs $SPCost SP." }
        if ($Player.Class -ne "Vampire") { $Player.SP -= $SPCost }; $Heal = 15; $Player.HP = [math]::Min($Player.MaxHP, ($Player.HP + $Heal))
    ...
    elseif ($SkillName -ieq 'First Aid') {
        ...
        if ($IsFreeCast) { $SPCost = 0 }
        if ($Player.SP -lt $SPCost -and $Player.Class -ne "Vampire") { return "Needs $SPCost SP." }
        if ($Player.Class -ne "Vampire") { $Player.SP -= $SPCost }
    ...
    elseif ($SkillName -ieq 'Cleave') {
        ...
        if ($null -ne $ExistingSunder) {
            ...
            $Mob.ActiveEffects += $Sunder; $Message += "You cleave the $($Mob.Name) for $DamageDealt damage, shattering their defenses!"
        }
    }
    elseif ($SkillName -ieq 'Vicious Kick') {
        ...
        if ((Get-Random -Min 1 -Max 101) -le 50) {
            $Mob.ActiveEffects += [PSCustomObject]@{ Name="Tactical Stun"; Duration=2; DoT=0; Modifiers=@{} }
    ...
    elseif ($SkillName -ieq 'Heads or Tails') {
        ...
        if ((Get-Random -Min 1 -Max 101) -le 51) {
            ...
            if ($null -eq $Player.ActiveEffects) { $Player.ActiveEffects = @() }
            $Player.ActiveEffects += [PSCustomObject]@{ Name="Tactical Stun"; Duration=1; DoT=0; Modifiers=@{} }
    ...
    elseif ($SkillName -ieq 'Brace') {
        ...
        if ($null -ne $ExistingBrace) {
            ...
            $Player.ActiveEffects += $Buff; $Message += "You brace yourself! Healed $Heal HP and gained +5 Armor for 3 turns."; $IsAttack = $false
        }
    }
    elseif ($SkillName -ieq 'Distract') {
        ...
        if ($null -ne $ExistingDistract) {
            ...
            $Mob.ActiveEffects += $DistractDebuff; $Message += "Your charisma distracts the $($Mob.Name)! They are mesmerized for $DistractTurns turn(s)."; $IsAttack = $false
        }
    }
    elseif ($SkillName -ieq 'Cancel') {
        ...
        if ($null -ne $ExistingShame) {
            ...
            $Mob.ActiveEffects += $Shame; $Message += "You absolutely cancel the $($Mob.Name) for $DamageDealt damage! They are Shamed (-5 DMG) for 3 turns."
        }
    }
    elseif ($SkillName -ieq 'Backstab') {
        ...
        if ($null -ne $ExistingBleed) {
            ...
            $Mob.ActiveEffects += $Bleed; $Message += "You backstab the $($Mob.Name) for $DamageDealt damage, leaving a gaping, bleeding wound!"
        }
    }
    elseif ($SkillName -ieq 'Exploit') {
        ...
        else { $Mob.ActiveEffects += [PSCustomObject]@{ Name="Exposed"; Duration=3; DoT=0; Modifiers=@{Armor=-5; Damage=-2} } }
        ...
        else { $Player.ActiveEffects += [PSCustomObject]@{ Name="Tactical Advantage"; Duration=3; DoT=0; Modifiers=@{TacticsChance=50} } }
    ...
    elseif ($SkillName -ieq 'Overload') {
        ...
        if ($null -ne $ExistingShock) {
            ...
            $Mob.ActiveEffects += $Shock; $Message += "You blast the $($Mob.Name) with overloaded tech for $DamageDealt damage! They are Shocked, taking 5 DoT and dealing less damage."
        }
    }
    elseif ($SkillName -ieq 'Lucky Strike') {
        ...
        if ($null -ne $ExistingLuck) {
            ...
            $Player.ActiveEffects += $LuckAura; $Message += "You swing blindly for $DamageDealt damage! An aura of extreme luck surrounds you."
        }
    }
    elseif ($SkillName -ieq 'Lob Tissue') {
        ...
        if ($Mob.IsBoss -or $Mob.IsImmune) {
            ...
            if ($null -ne $ExistingCripple) {
                ...
                $Mob.ActiveEffects += $Cripple; $Message += "`n> COIN FLIP FAILED: The target is Crippled (-4 DMG, -2 AC)!"
            }
        }
    }
    elseif ($SkillName -ieq 'Toxic Cloud') {
        ...
        # --- PLAGUEBRINGER 20: LEGENDARY TOXIN ---
        ...
        if ($IsImmune) {
            ...
            # Bypassed via legendary toxin, do nothing further
        } elseif (Invoke-InfectionCheck -Infectivity $Player.Infectivity) {
            $DamageDealt = 9999
            if ($null -eq $Player.ActivePet) {
                $Player.ActivePet = [PSCustomObject]@{ Name="Infected $($Mob.Name)"; Level=$Mob.Level; XP=0; MaxHP=($Mob.MaxHP + $Player.PetBonusHP); HP=($Mob.MaxHP + $Player.PetBonusHP); Damage=$Mob.Damage; Armor=$Mob.Armor; IsLegacyPet = $false }
            ...
            if ($null -ne $ExistingRot) {
                ...
                $Mob.ActiveEffects += $Rot; $Message += "`n> COIN FLIP FAILED: The target suffers heavy Zombie Rot!"
            }
        }
    }
    elseif ($SkillName -ieq 'Transform') {
        if ($null -eq $Player.ActiveEffects) { $Player.ActiveEffects = @() }
        $AlreadyTransformed = @($Player.ActiveEffects) | Where-Object { $_.Name -eq "Hulk Form" } | Select-Object -First 1
    ...
    elseif ($SkillName -ieq 'Blood Curse') {
        ...
        if ($null -ne $ExistingCurse) {
            ...
            $Mob.ActiveEffects += $Curse; $Message += "You expend $BPCost BP to inflict a Blood Curse! The target takes heavy DoT and is weakened. If it dies, it will rise as your servant!"; $IsAttack = $false
        }
    }
    elseif ($SkillName -ieq 'Blood Mist') {
        ...
        if ($null -eq $Player.ActiveEffects) { $Player.ActiveEffects = @() }
        $Player.ActiveEffects += [PSCustomObject]@{ Name="Blood Mist"; Duration=5; DoT=0; Modifiers=@{Armor=10; Damage=$BonusDmg} }
        $Player.ActiveEffects += [PSCustomObject]@{ Name="Exhausted"; Duration=10; DoT=0; Modifiers=@{} }
    ...
    elseif ($SkillName -ieq 'Exsanguinate') {
        ...
        $Mob.ActiveEffects += [PSCustomObject]@{ Name="Bleeding"; Duration=4; DoT=12; Modifiers=@{} }
        $Mob.ActiveEffects += [PSCustomObject]@{ Name="Exposed"; Duration=3; DoT=0; Modifiers=@{Armor=-5; Damage=-2} }
        $Mob.ActiveEffects += [PSCustomObject]@{ Name="Crippled"; Duration=3; DoT=0; Modifiers=@{Damage=-4; Armor=-2} }
    ...
    elseif ($SkillName -ieq 'Firearm') {
        ...
        if ($HasShotgun) {
            ...
            if ($null -eq $Mob.ActiveEffects) { $Mob.ActiveEffects = @() }
            $ExistingBleed = @($Mob.ActiveEffects) | Where-Object { $_.Name -eq "Bleeding" } | Select-Object -First 1
            if ($null -ne $ExistingBleed) { $ExistingBleed.Duration = 4 } else { $Mob.ActiveEffects += [PSCustomObject]@{ Name="Bleeding"; Duration=4; DoT=12; Modifiers=@{} } }
        ...
        if ($null -ne $ExistingAdrenaline) {
            ...
            $Player.ActiveEffects += $Adrenaline; $Message += "`n> The panic gives you an Adrenaline rush (+DEX, +TAC, +AC)."
        }
    }
                ...
                switch ($Player.ActivePet.Role) {
                    ...
                    "Buff" {
                        ...
                        if ($null -ne $ExistingAura) {
                            ...
                            $Player.ActiveEffects += $Buff; $Message += "`n> Your Legacy $($Player.ActivePet.Name) grants a battle aura! +$BuffStr STR, +$BuffAC AC."
                        }
                    }
                    "Damage" {
                        if ($Mob.HP -gt 0) {
    ...
    if ($Mob.HP -le 0) {
        ...
        # The AI turn will be handled automatically by Client.ps1 after the skill finishes!
    }
