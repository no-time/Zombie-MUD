    ...
    if ((Get-Random -Minimum 1 -Maximum 101) -gt $MobHitChance) {
        ...
        if ($UseSkill) {
            ...
            switch ($ChosenSkill) {
                ...
                { $_ -eq "Transform" } {
                    if ($null -eq $Mob.ActiveEffects) { $Mob.ActiveEffects = @() }
                    $ExistingHulk = @($Mob.ActiveEffects) | Where-Object { $_.Name -eq "Hulk Form" } | Select-Object -First 1
                    $ExistingExhaustion = @($Mob.ActiveEffects) | Where-Object { $_.Name -eq "Exhausted" } | Select-Object -First 1

                    if ($null -ne $ExistingHulk -or $null -ne $ExistingExhaustion) {
                        $Message += "`n> The $($Mob.Name) tries to mutate again, but its body is too exhausted to handle the strain!"
                    } else {
                        $Mob.HP = $Mob.MaxHP
                        $Buff = [PSCustomObject]@{ Name="Hulk Form"; Duration=10; DoT=0; Modifiers=@{Damage=20} }
                        $Exhaustion = [PSCustomObject]@{ Name="Exhausted"; Duration=13; DoT=0; Modifiers=@{} }
                        $Mob.ActiveEffects = @($Mob.ActiveEffects) + $Buff + $Exhaustion
                        $Message += "`n> The $($Mob.Name) mutates into a raging HULK! HP fully restored and gains +20 Damage. (Causes 13 turns of Exhaustion)"
                    }
                }
